...
const words = {
    "karisikHarfler": [
        ...
        "okul", "top", "park", "kalem", "defter", "çiçek", "gitar", "televizyon", "kutu", "adam", "eldiven",
        "kar", "ışık", "çanta", "mutfak", "halı", "resim", "ağaç", "şemsiye", "tencere", "ayakkabı", "kuş",
        "balık", "yıldız", "deniz", "güneş", "bulut", "ay", "kumar", "sinema", "tiyatro", "uçak", "portakal",
        "muz", "çilek", "dut", "armut", "nar", "kalorifer", "buzdolabı", "şarj", "dolap", "masaüstü", "klavye",
        "fare", "bilim", "radyo", "pazartesi", "salı", "çarşamba", "perşembe", "cuma", "cumartesi", "pazar", "yemek",
        "çorba", "salata", "börek", "mavi", "yeşil", "beyaz", "sarı", "mor", "turuncu", "gri", "siyah", "kahverengi",
        "taş", "toprak", "su", "hoca", "öğrenci", "şeker", "tatlı", "tuzlu", "acı", "balon", "kamyon", "jip", "sandal",
        "gemi", "otobüs", "tren", "helikopter", "bahçe", "orman", "şehir", "köy", "ev", "apartman", "daire", "oda",
        "balkon", "çatı", "duvar", "kapı", "koltuk", "sandık", "yatak", "kasa", "ayna", "tablo", "zincir",
        "dergi", "gazete", "film", "lamba", "keman", "flüt", "davul","piyano", "fotoğraf", "heykel",
        "yazar", "kitap", "şiir", "roman", "hikaye", "masal", "tarih", "coğrafya", "fizik", "kimya", "biyoloji",
        "matematik", "geometri", "felsefe", "edebiyat", "dil", "din", "sanat", "müzik", "spor", "futbol",
        "basketbol", "voleybol", "yüzme", "tenis", "golf", "satranç", "hentbol", "kano", "sörf", "kayak",
        "kırmızı", "limon", "greyfurt", "mandalina", "kivi", "avokado", "havuç", "domates", "salatalık", "patlıcan", "soğan",
        "biber", "marul", "karnabahar", "brokoli", "fasulye", "bezelye", "nohut", "mercimek", "pirinç", "bulgur",
        "kepek", "kinoa", "çörekotu", "ceviz", "fındık", "badem", "zeytin", "peynir", "tereyağı", "süt", "yoğurt", "kaymak",
        "dondurma", "bal", "çikolata", "reçel", "pekmez", "tahin", "helva", "üzüm", "incir", "kayısı", "erik", "şeftali",
        "kavun", "karpuz", "vişne", "kiraz"
    ],
    "ingTurkKelime": {
    ...
    "zitAnlamli": {
        ...
        "sık": "seyrek", "başlamak": "bitirmek", "gelir": "gider", "somut": "soyut",
...
// Geri sayımı ve oyunu başlatma fonksiyonu
function startGame(gameType) {
    document.querySelector('.game-selection').classList.add('hidden');
    document.getElementById('game-area').style.display = 'block';

    // Kullanıcı girişlerini devre dışı bırak
    document.getElementById('user-input').disabled = true;
    document.getElementById('hint-button').disabled = true;
    document.getElementById("skipButton").disabled = true;

    // Pas geçme ve ipucu bilgilerini sıfırla
    document.getElementById("skipButton").textContent = "Pas Geç";
    document.getElementById("passesDisplay").innerText = "Kalan pas geçme hakkı: 3";
    hintUsed = false;

    // Oyun açıklamasını belirle
    setDescription(gameType);

    // Skor ve zaman sıfırla
    ...
    // İlk geri sayım mesajını göster
    const countdownDisplay = document.getElementById('countdown');
    let countdownTime = 5;
    countdownDisplay.textContent = `Oyunun başlamasına ${countdownTime} saniye kaldı...`;
    ...
    const countdownInterval = setInterval(() => {
        ...
        if (countdownTime >= 0) {
            ...
            countdownDisplay.textContent = ""; // Geri sayım mesajını temizle
            startPlayingGame(gameType); // Oyunu başlat
        }
    }, 1000);
}

function startPlayingGame(gameType) {
    // Kullanıcı girişlerini etkinleştir
...
function updateTimer() {
    ...
    if (timeLeft <= 0) {
        ...
        // Oyunun bitmesiyle input ve butonları devre dışı bırak
        document.getElementById('user-input').disabled = true; // Kullanıcının cevabını girmesini engelle
        document.getElementById('skipButton').disabled = true; // Pas geçme butonunu devre dışı bırak
        document.getElementById('hint-button').disabled = true; // İpucu butonunu devre dışı bırak
    }
}
...
let remainingPasses = 3; // Başlangıçta 3 pas geçme hakkı

// Pas geçme butonunun işlevi
function skipQuestion() {
    if (remainingPasses > 0) {
        remainingPasses--; // Her pas geçişinde hakkı azalt
        loadNextQuestion(); // Yeni soruya geç
        document.getElementById("passesDisplay").innerText = `Kalan pas geçme hakkı: ${remainingPasses}`;

        // Pas geçme hakkı 0 olduğunda butonu devre dışı bırak
        if (remainingPasses === 0) {
            document.getElementById("skipButton").disabled = true;
            document.getElementById("skipButton").textContent = "Pas geçme hakkınız bitti.";
        }
    }
}
